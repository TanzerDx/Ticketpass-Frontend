      ...
      UserService.getUserByAccessToken(accessToken)
        ...
        .then((data) => {
          const filteredOrders = data.orders.filter(
            (order) => new Date(order.concert.date) > new Date()
          );
      ...
      <div className="background-color-orders">
        <div className="content">
          <div className="orders-heading">
            {user ? (
              <h1
                data-testid="cypress-orders-upcomingConcerts"
                className="orders-heading-margin"
              >
                UPCOMING CONCERTS
              </h1>
            ) : (
              ...
              orders.map((order) => (
                <div className="order-container" key={order.id}>
                  <div className="order-artist-name">
                    <h1 className="orders-heading-margin">
                  ...
                  <div className="order-concert-details">
                    <h1 className="orders-heading-margin">
                      {format(new Date(order.concert.date), "dd-MM-yyyy HH:mm")}
                    </h1>
                  </div>

                  <button
                    type="button"
                    className="button-showTickets"
                    name="showTickets"
                    onClick={() => handleGetTickets(order.id)}
                  >
                    SHOW TICKETS
                  </button>
                </div>
