describe("Purchase Concert Tickets", () => {
  describe("Successfully obtained concert tickets", () => {
    beforeEach(() => {
      // Log In
      ...
      cy.get('[data-testid="cypress-selectConcert-form"]')
        .first()
        .click({ force: true });
      ...
      cy.get('[data-testid="cypress-checkout-input-name"]').should(
        "have.value",
        "Name"
      );

      cy.get('[data-testid="cypress-checkout-input-surname"]').type("Surname");
      cy.get('[data-testid="cypress-checkout-input-surname"]').should(
        "have.value",
        "Surname"
      );

      cy.get('[data-testid="cypress-checkout-input-address"]').type(
        "Rachelsmolen 1"
      );
      cy.get('[data-testid="cypress-checkout-input-address"]').should(
        "have.value",
        "Rachelsmolen 1"
      );

      cy.get('[data-testid="cypress-checkout-input-phoneNumber"]').type(
        "+31612345678"
      );
      cy.get('[data-testid="cypress-checkout-input-phoneNumber"]').should(
        "have.value",
        "+31612345678"
      );
    ...
    it("displays the purchased concert ticket", () => {
      cy.url().should("include", "http://localhost:5173/tickets");

      cy.get('[data-testid="cypress-ticket-object"]').should("exist");

      cy.get('[data-testid="cypress-ticket-artistName"]')
        .should("exist")
        .should("have.text", "Taylor Swift");

      cy.get('[data-testid="cypress-ticket-id"]').should("exist");
  ...
  describe("Unsuccessful attempt to purchase tickets", () => {
    beforeEach(() => {
      // Log In
      cy.visit(`http://localhost:5173/signin`);
      cy.viewport(1920, 1080);
    ...
    it("displays an error that the TOS checkbox is not checked", () => {
      cy.url().should("include", "http://localhost:5173/checkout");

      cy.get(".cypress-toastify-error-checkboxNotChecked").should(
        "contain",
        "Please agree to the General Terms of Ticketpass before proceeding!"
      );
    });
  });
});
